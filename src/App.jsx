...
function VisionLinkSyncButton({activeMine}){
  ...
  const sync=async()=>{setSyncing(true);setErr("");setMsg("");try{if(!activeMine?.id)throw new Error("No active mine");const{data:{session}}=await supabase.auth.getSession();const url=`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/visionlink-sync`;const res=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${session?.access_token||import.meta.env.VITE_SUPABASE_ANON_KEY}`,"apikey":import.meta.env.VITE_SUPABASE_ANON_KEY},body:JSON.stringify({mine_id:activeMine.id})});const body=await res.json().catch(()=>({error:`HTTP ${res.status}`}));if(!res.ok||body?.error)throw new Error(body?.error||`HTTP ${res.status}`);setMsg(`✓ Synced ${body?.updated||0} of ${body?.total_assets||0} machines`);setTimeout(()=>setMsg(""),4000);}catch(e){setErr(e.message||"Sync failed");setTimeout(()=>setErr(""),6000);}finally{setSyncing(false);}};

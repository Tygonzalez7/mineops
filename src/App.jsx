...
function VisionLinkSyncButton({activeMine}){
  ...
  const sync=async()=>{setSyncing(true);setErr("");setMsg("");try{if(!activeMine?.id)throw new Error("No active mine");const{data,error}=await supabase.functions.invoke("visionlink-sync",{body:{mine_id:activeMine.id}});if(error){let detail=error.message;try{const ctx=await error.context?.json?.();if(ctx?.error)detail=ctx.error;}catch{}throw new Error(detail);}if(data?.error)throw new Error(data.error);setMsg(`✓ Synced ${data?.updated||0} of ${data?.total_assets||0} machines`);setTimeout(()=>setMsg(""),4000);}catch(e){setErr(e.message||"Sync failed");setTimeout(()=>setErr(""),6000);}finally{setSyncing(false);}};
